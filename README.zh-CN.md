...
- `get_announcement_detail`：解析/标准化巨潮公告详情链接，可选提取限定长度 PDF 文本预览，并返回文本质量指标。
...
- 公告 PDF 文本抽取是 best-effort；当 `text_status` 为 `poor_quality` 时，不能把 `text` 当成可靠全文，应以 `pdf_url` 原文为准。
