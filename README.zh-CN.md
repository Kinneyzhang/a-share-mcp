...
- `batch_get_quotes`：批量行情快照，单只失败不影响整体返回。
- `batch_company_snapshot`：批量公司快照。
- `compare_companies`：多公司指标对比和简单排名。
- `screen_stocks`：按行业、市值、PE 等公开字段做简单筛选。
- `get_market_overview`：主要指数、行业板块、概念板块概览。
- `get_financial_trends`：基于财务指标的趋势摘要。
- `classify_announcements`：公告关键词分类摘要。
- `get_cache_status` / `clear_cache`：查询或清理本地 JSON 缓存。
...
从 `v1.0.0` 开始，已有 tool 名称和参数名按稳定 API 处理：

- patch 版本只修 bug，不删除字段。
- minor 版本可以增加工具、可选参数或响应字段。
- 破坏性变更必须进入新的 major 版本，并提供迁移说明。
